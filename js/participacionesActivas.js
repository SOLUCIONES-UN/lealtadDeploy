...
function mostrarDatosEnTabla(datos) {
  ...
  datos.forEach((element) => {
    element.participaciones.forEach((participacion) => {
      ...
      const telefono = participacion.customerInfo
        ? participacion.customerInfo.telno
        : "Desconocido";
      const descripcionTrx = participacion.descripcionTrx || "Sin descripción";
      const valor = participacion.valor || "Sin valor";
      const nombre = participacion.customerInfo
        ? participacion.customerInfo.fname
        : "Sin nombre";
      const codigo = participacion.customerInfo
        ? participacion.customerInfo.customer_reference
        : "Sin código";
      const nombreCampania = participacion.campanium
        ? participacion.campanium.nombre
        : "Sin campaña";
      const fechaCreacion = participacion.campanium
        ? participacion.campanium.fechaCreacion
        : "Sin fecha";
      const premioDescripcion = participacion.premioDescripcion || "Sin premio";
      const premioMonto =
        participacion.detallepromocion &&
        participacion.detallepromocion.premiopromocion
          ? parseFloat(
              participacion.detallepromocion.premiopromocion.valor
            ).toFixed(2)
          : "0.00";
      const cupon = participacion.detallepromocion
        ? participacion.detallepromocion.cupon
        : "Sin cupón";
  ...
  .addEventListener("click", function () {
    ...
    const headerRow1 = [
      { v: "", t: "s", s: { font: { name: "Courier", sz: 18 } } },
      {
        v: "",
        t: "s",
        s: { font: { sz: 18 }, alignment: { horizontal: "center" } },
      },
      {
        v: "",
        t: "s",
      ...
      {
        v: " REPORTE DE NOTIFICACIONES  OFFERCRAFT",
        t: "s",
        s: { font: { sz: 18 }, alignment: { horizontal: "center" } },
      },
    ];
