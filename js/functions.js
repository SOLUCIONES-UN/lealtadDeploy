...
const initDateInputs = () => {
  ...
  dateInputs.forEach(
    (input) => (input.value = new Date(0).toISOString().substring(0, 10))
  );
};
...
const checkFormVals = (action) => {
  ...
  Object.entries(allFormVal).length == trueVals
    ? (allFormIsOK = true)
    : (allFormIsOK = false);
...
const invalidFormData = () =>
  Alert("Por favor completar los datos correctamente.", "error");
    ...
    () =>
      setTimeout(() =>
        isNan(input.value) < 1
          ? setValidFormData(
              [input.id, false],
              Alert(`${message} debe ser de al menos 1.`, "error")
            )
          : setValidFormData([input.id, true], removeLeadingZeroes(input.value))
      ),
    100
  );
...
const validateAge = (input, message) =>
  input.addEventListener(
    ...
    () =>
      setTimeout(() =>
        isNan(input.value) < 1 || isNan(input.value) > 99
          ? setValidFormData(
              [input.id, false],
              Alert(
                `${message} debe encontrarse en un rango entre 1 y 99 años.`,
                "error"
              )
            )
          : setValidFormData([input.id, true], removeLeadingZeroes(input.value))
    ...
    setTimeout(
      () =>
        isEmpty(input.value)
          ? setValidFormData(
              [input.id, false],
              Alert(`${message} no puede estar vacío.`, "error")
            )
          : setValidFormData([input.id, true]),
      100
    )
  );
...
const validateDate = (input, message) =>
  input.addEventListener("input", () =>
    setTimeout(
      () =>
        input.value.length == 0 || input.value == "1970-01-01"
          ? setValidFormData(
              [input.id, false],
              Alert(`${message} no tiene una fecha válida.`, "error")
            )
          : setValidFormData([input.id, true]),
      100
    )
    ...
    setTimeout(
      () =>
        input.value == "0"
          ? setValidFormData(
              [input.id, false],
              Alert(`${message} debe tener una opción seleccionada.`, "error")
            )
          : setValidFormData([input.id, true]),
      100
    )
    ...
    setTimeout(
      () =>
        input.files.length == 0
          ? setValidFormData(
              [input.id, false],
              Alert(`${message} debe de contener un archivo.`, "error")
            )
          : setValidFormData([input.id, true]),
      100
    )
