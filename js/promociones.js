            ...
            <a href="#" onclick="OpenDelete(${data})" class="btn_delete dropdown-item">
                ${feather.icons["trash-2"].toSvg({
                  class: "font-small-4 mr-50",
                    ...
                    <a 
                    <button class="btn btn-success BtnExcel" onclick="generarExcel(${data})">
    <span class="align-middle d-sm-inline-block d-none">Codigos en Excel</span>
...
document.querySelector('.BtnExcel').addEventListener('click', function() {
    // Aquí obtén los datos necesarios para generar el archivo Excel
    var data = obtenerDatosParaExcel();

    // Llama a la función para generar el archivo Excel con los datos obtenidos
    generarExcel(data);
});

function obtenerDatosParaExcel() {
    // Aquí obtén los datos necesarios para generar el archivo Excel
    // Por ejemplo, si los datos están en algún elemento HTML, puedes recuperarlos de la misma manera que en la función OpenEdit()
    // Luego, devuelves los datos en el formato adecuado para la generación del Excel
}
...
function generarExcel(data) {
    ...
    // Aquí inserta los datos recibidos en el archivo Excel
    // Puedes usar la misma lógica que en la función generarExcel() original
